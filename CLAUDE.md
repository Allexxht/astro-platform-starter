...
- **Külön job (`cross-tenant-rollback-test`)** minden migrációt a hozzá tartozó `db/migrations/rollback/`-beli visszaállító scripttel EGYÜTT is kipróbál, saját eldobható stacken: a migráció előtti alapállapotot a PR/push bázis commitjáról veszi (`git show <bázis>:db/supabase-setup.sql`), majd a migráció + rollback kör után bájtra pontosan összeveti a táblák/függvények/policy-k listáját és minden tábla sorszámát az eredetivel – bukik, ha bármi eltér (séma nem állt vissza pontosan, vagy adat veszett el).
